...
function(GenerateSimulateScript)
    ...
    if(KernelArchX86)
        ...
        set(script "qemu-system-x86_64 -cpu ${sim_cpu} -m 512 -nographic -serial mon:stdio -kernel ${KERNEL_IMAGE_NAME} -initrd ${IMAGE_NAME}")
    elseif(KernelPlatformKZM)
        set(script "qemu-system-arm -nographic -M kzm -kernel ${IMAGE_NAME}")
    elseif(KernelPlatformSabre)
        set(script "qemu-system-arm -machine sabrelite -nographic -m size=1024M -s -serial null -serial mon:stdio -kernel ${IMAGE_NAME}")
    elseif(KernelPlatformZynq7000)
        set(script "qemu-system-arm -machine xilinx-zynq-a9 -nographic -m size=1024M -s -serial null -serial mon:stdio -kernel ${IMAGE_NAME}")
    elseif(KernelPlatformWandQ)
        set(script "qemu-system-arm -machine sabrelite -nographic -m size=2048M -s -serial mon:stdio -kernel ${IMAGE_NAME}")
    else()
