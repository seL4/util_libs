...
def report(data=None, use_c=False) -> str:
    """
    ...
    if use_c:
        ...
        head = '''{comment_begin}
int num_memory_regions = {n};

struct memory_region {{
    size_t start;
    size_t end;
}} memory_region[{n}] = {{
'''.format(comment_begin=comment_template.format(tag='BEGIN'), n=n)
        regions = []
...
def _process_operand(yaml_file: str, use_c: bool) -> bool:
    """
...
Future directions: Add command-line options to support user-specified names for
the generated C symbols (array length, array, and structure tag).
""")
    ...
    for yaml_file in args.platform_file:
        if not _process_operand(yaml_file, use_c=args.emit_c_syntax):
