...
static freq_t _peric_clk_get_freq(clk_t *clk)
{
    freq_t fin;
    int clkid;
    int div;
...
static freq_t _peric_clk_set_freq(clk_t *clk, freq_t hz)
{
    freq_t fin;
    int clkid;
    int div;
...
static void _peric_clk_recal(clk_t *clk)
{
    assert(!"Not implemented");
}

static clk_t *_peric_clk_init(clk_t *clk)
{
...
static int exynos5_gate_enable(clock_sys_t *clock_sys, enum clock_gate gate, enum clock_gate_mode mode)
{
    return -1;
}
