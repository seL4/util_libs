...
uint64_t rk_get_time(rk_t *rk)
{
    ...
    start = rk->hw->load_count0 + ((uint64_t)rk->hw->load_count1 << 32);
    time = start - time;
...
int rk_start_timestamp_timer(rk_t *rk)
{
    ...
    //set timer to count down monotonically
...
int rk_init_secondary(rk_t *rk, rk_t *rkp, ps_io_ops_t ops, rk_config_t config)
{
    ...
    rk->rk_map_base = NULL;
    /* just like dmt, rockchips have another timer in the same page at 0x20 offset */
