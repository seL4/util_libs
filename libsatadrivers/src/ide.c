...
typedef struct ide_dev {
    struct IDEChannelRegisters channels[NUM_IDE_CHANNELS];
    ps_io_ops_t *io_ops;
} ide_dev_t;
 ...
 *
 */
uint8_t ide_polling(ide_dev_t *ide, uint8_t channel, uint32_t advanced_check)
{
    uint8_t err = ATA_NO_ERR;
    uint32_t res = 0;
    uint32_t bus = ide->channels[channel].base;      /* Bus Base, like 0x1F0 which is also data port. */
    uint32_t ctrl = ide->channels[channel].ctrl; /* Control Base, like 0x1F0 which is also data port. */

    ps_io_ops_t *io_ops = ide->io_ops;
 ...
 *
 */
uint8_t ide_print_error(sata_driver_t *driver, uint32_t drive, uint8_t err)
{
    if (NULL == driver) {
        ZF_LOGE("IDE: print_error: driver can't be NULL!");
        return INV_ADDR_ERR;
    }

    if (driver->mode != IDE) {
        ZF_LOGE("IDE: print_error: calling print_error on a SATA driver not designated as IDE");
        return UNCORRECT_DATA_ERR;
    }

    if (!(0 <= drive && drive < MAX_DRIVES)) {
        ZF_LOGE("IDE: print_error: drive can only be between 0 and %d", MAX_DRIVES);
        return INV_ADDR_ERR;
    }

    ide_dev_t *ide = driver->driver;
    ps_io_ops_t *io_ops = ide->io_ops;

    /* Bus Base, like 0x1F0 which is also data port. */
    uint32_t bus = ide->channels[driver->sata_devices[drive].Channel].base;
    if (ATA_NO_ERR == err) {
        return err;
    }
    ...
    (const char *[]) {
        "Primary", "Secondary"
    }[driver->sata_devices[drive].Channel],
    (const char *[]) {
        "Master", "Slave"
    }[driver->sata_devices[drive].Drive],
    driver->sata_devices[drive].Model);
 ...
 * Returns: success (0) failure (code)
 */
uint8_t ide_ata_access(sata_driver_t *driver, uint8_t direction, uint8_t drive, uint32_t lba,
                       uint16_t numsects, uint8_t *buf)
{
    if (NULL == driver) {
        ZF_LOGE("IDE: ata_access: driver can't be NULL!");
        return INV_ADDR_ERR;
    }

    if (driver->mode != IDE) {
        ZF_LOGE("IDE: ata_access: calling ata_access on a SATA driver not designated as IDE");
        return UNCORRECT_DATA_ERR;
    }

    if (!(0 <= drive && drive < MAX_DRIVES)) {
        ZF_LOGE("IDE: ata_access: drive can only be between 0 and %d", MAX_DRIVES);
        return INV_ADDR_ERR;
    }
    ...
    uint32_t  channel = driver->sata_devices[drive].Channel;  /* Read the Channel */
    uint32_t  slavebit = driver->sata_devices[drive].Drive;   /* Read the Drive [Master/Slave] */
    /* Bus Base, like 0x1F0 which is also data port. */
    uint32_t  bus = ide->channels[channel].base;
    /* Control Base, like 0x1F0 which is also data port. */
    uint32_t  ctrl = ide->channels[channel].ctrl;
    uint16_t  cyl, i;
    uint8_t   head, sect;
    uint32_t  bufp_val = 0;
    ...
    } else if (driver->sata_devices[drive].Capabilities & 0x200) {
        // LBA28:
    ...
    if (ATA_READ == direction) {
        ...
        for (i = 0; i < numsects; i++) {
            ...
            error = ide_polling(ide, channel, 1);
            if (error) {
                return error;
            }
 ...
 *
 */
int ide_init(ps_io_ops_t *io_ops, sata_driver_t *driver)
{
    if (NULL == driver) {
        ZF_LOGE("IDE: init: driver can't be NULL!");
        return INV_ADDR_ERR;
    }

    if (driver->mode != IDE) {
        ZF_LOGE("IDE: init: calling init on a SATA driver not designated as IDE");
        return UNCORRECT_DATA_ERR;
    }
    ...
    error = ps_calloc(&io_ops->malloc_ops, 1, sizeof(ide_dev_t), (void **) &driver->driver);
    if (error) {
        ZF_LOGE("IDE: init: failed to allocate mmeory for IDE driver");
        return INV_ADDR_ERR;
    }
    ...
    ide->channels[ATA_PRIMARY].base = (PRIMARY_CMD_BASE);
    ide->channels[ATA_PRIMARY].ctrl = (PRIMARY_CTRL_BASE);

    /* Disable IRQs */
    error = ps_io_port_out(&io_ops->io_port_ops, (uint32_t)(ide->channels[ATA_PRIMARY].ctrl
    ...
    for (i = 0; i < NUM_IDE_CHANNELS; i++) {
        ...
        driver->sata_devices[count].Reserved = 0; /* Assuming that no drive here. */

        /* Select Drive */
        error = ps_io_port_out(&io_ops->io_port_ops, (uint32_t)(ide->channels[ATA_PRIMARY].base
        ...
        /* Polling */
        error = ps_io_port_in(&io_ops->io_port_ops, (uint32_t)(ide->channels[ATA_PRIMARY].base
        ...
        driver->sata_devices[count].Type         = type;
        driver->sata_devices[count].Channel      = 0;
        driver->sata_devices[count].Drive        = i;
        memcpy(&driver->sata_devices[count].Signature, ide_buf + ATA_IDENT_DEVICETYPE, sizeof(uint16_t));
        memcpy(&driver->sata_devices[count].Capabilities, ide_buf + ATA_IDENT_CAPABILITIES, sizeof(uint16_t));
        memcpy(&driver->sata_devices[count].CommandSets, ide_buf + ATA_IDENT_COMMANDSETS, sizeof(uint32_t));

        /* Get Size */
        if (driver->sata_devices[count].CommandSets & (1 << 26)) {
            /* Device uses 48-Bit Addressing */
            memcpy(&driver->sata_devices[count].Size, ide_buf + ATA_IDENT_MAX_LBA_EXT, sizeof(uint32_t));
        } else {
            /* Device uses CHS or 28-bit Addressing */
            memcpy(&driver->sata_devices[count].Size, ide_buf + ATA_IDENT_MAX_LBA, sizeof(uint32_t));
        }

        /* String indicates model of device (like Western Digital HDD and SONY DVD-RW...) */
        for (k = 0; k < MODEL_STRING_SIZE - 1; k += 2) {
            driver->sata_devices[count].Model[k] = ide_buf[ATA_IDENT_MODEL + k + 1];
            driver->sata_devices[count].Model[k + 1] = ide_buf[ATA_IDENT_MODEL + k];
        }
        driver->sata_devices[count].Model[MODEL_STRING_SIZE - 1] = 0; // Terminate String.
    ...
    /* Print Summary */
    for (i = 0; i < NUM_IDE_CHANNELS; i++) {
        if (1 == driver->sata_devices[i].Reserved) {
            ZF_LOGV(" (%d) Found %s Drive %dGB - %s\n", i,
            (const char *[]) {
                "ATA", "ATAPI"
            }[driver->sata_devices[i].Type],  /* Type */
            driver->sata_devices[i].Size / 1024 / 1024 / 2,                  /* Size */
            driver->sata_devices[i].Model);
        }
    }
